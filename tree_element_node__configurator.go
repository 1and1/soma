	...
	constraintcheck:
		...
		for svcId, _ := range serviceC {
			...
			for _, y := range results {
				...
				// check existing for same ConstraintHash
				// ... same ConstraintValHash
				// ... ... same InstanceSvcCfgHash --> instance update
				// ... same InstanceSvcCfgHash     --> instance update (new constraints)
				// add new instances

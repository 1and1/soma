...
func sqlInventoryTables01() {
  var err error;

  _, err = db.Exec(`create table if not exists inventory.datacenters (
    datacenter                  varchar(32)     PRIMARY KEY
  );`); if err != nil {
    log.Fatal( err )
  }

  _, err = db.Exec(`create table if not exists inventory.servers (
  ...
  db.Exec(`drop index _unique_server_online;`);

  _, err = db.Exec(`create unique index _unique_server_online
...
func sqlInventoryTables02() {
  var err error;

  _, err = db.Exec(`create table if not exists inventory.organizational_teams (
  ...
  _, err = db.Exec(`create table if not exists inventory.oncall_duty_teams (
  ...
  _, err = db.Exec(`create table if not exists inventory.users (
  ...
  _, err = db.Exec(`create table if not exists inventory.oncall_duty_membership (

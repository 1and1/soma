...
func main() {
	...
	if !SomaCfg.ReadOnly {
		if !SomaCfg.Observer {
			...
			router.PATCH(`/authenticate/user/password/:uuid`, AuthenticationChangeUserPassword)
			...
			router.PUT(`/authenticate/user/password/:uuid`, AuthenticationResetUserPassword)
